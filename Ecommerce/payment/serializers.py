...
class CheckoutSerializer(serializers.ModelSerializer):
    ...
    def update(self, instance, validated_data):
        ...
        if not instance.shipping_address:
            order_shipping_address = UserAddress(**shipping_address)
            order_shipping_address.save()
        else:
            # Shipping address for an order is already set so update its value
            address = UserAddress.objects.filter(shipping_orders=instance.id)
        ...
        if not instance.billing_address:
            order_billing_address = UserAddress(**billing_address)
            order_billing_address.save()

        else:
            # Billing address is set so update its value
            address = UserAddress.objects.filter(billing_orders=instance.id)
